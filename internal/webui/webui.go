...
function renderMembers(items,depth=0){if(!Array.isArray(items)||!items.length)return "";return '<div class="members">'+items.map(m=>'<div class="member"><div class="member-head"><strong>'+esc(m.name||"member")+'</strong>'+badge(m.verdict)+'</div>'+field("Format",m.format)+field("Size",humanBytes(m.size))+field("SHA-256",m.sha256,true)+(m.detection?'<p class="detection">'+esc(m.detection)+'</p>':"")+(m.error?'<p class="bad-text">'+esc(m.error)+'</p>':"")+renderMembers(m.children,depth+1)+jsonBlock("Member analysis",Object.fromEntries(Object.entries(m).filter(([k])=>!["name","size","sha256","format","verdict","detection","error","children"].includes(k))))+'</div>').join("")+'</div>';}
function renderResult(r){const known=new Set(["path","name","size","sha256","format","verdict","detection","archive","preservation_image","member_results","adf","filesystem","hunk","bootblock_match"]);const extra=Object.fromEntries(Object.entries(r).filter(([k])=>!known.has(k)));return '<div class="result-hero"><div><span class="eyebrow">Aggregate verdict</span><div class="verdict">'+badge(r.verdict)+'</div></div><div class="hero-meta">'+field("Format",r.format)+field("Size",humanBytes(r.size))+field("SHA-256",r.sha256,true)+'</div></div>'+(r.detection?'<div class="alert"><strong>Detection</strong><p>'+esc(r.detection)+'</p></div>':"")+'<div class="result-grid"><section class="result-card"><h3>Native AAA analysis</h3>'+jsonBlock("Bootblock match",r.bootblock_match)+jsonBlock("ADF analysis",r.adf)+jsonBlock("Filesystem analysis",r.filesystem)+jsonBlock("Hunk analysis",r.hunk)+( !r.bootblock_match&&!r.adf&&!r.filesystem&&!r.hunk?'<p class="muted">No format-specific native analysis details.</p>':"")+'</section><section class="result-card"><h3>Container / preservation</h3>'+jsonBlock("Archive analysis",r.archive)+jsonBlock("Preservation image",r.preservation_image)+(!r.archive&&!r.preservation_image?'<p class="muted">Not an archive or preservation-image result.</p>':"")+'</section></div>'+(Array.isArray(r.member_results)&&r.member_results.length?'<section class="result-card wide"><h3>Archive members</h3>'+renderMembers(r.member_results)+'</section>':"")+(Object.keys(extra).length?'<section class="result-card wide"><h3>Additional attributed evidence</h3><p class="muted">Fields added by later engines or API revisions are preserved here.</p>'+jsonBlock("Additional evidence",extra)+'</section>':"")+'<details class="raw"><summary>Raw API result</summary><pre>'+esc(JSON.stringify(r,null,2))+'</pre></details>';}
...
const styleCSS = `:root{font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color-scheme:dark;background:#111827;color:#e5e7eb}*{box-sizing:border-box}body{margin:0;background:#0b1020}header,main,footer{max-width:1100px;margin:auto}header{display:flex;align-items:center;justify-content:space-between;padding:2rem 1rem 1rem}h1{font-size:2rem;margin:0}h2,h3{margin:.1rem 0 .35rem}p{margin:.2rem 0}.muted{color:#9ca3af}.badge,.state{display:inline-block;border-radius:999px;padding:.3rem .65rem;background:#374151;font-size:.85rem}.good,.succeeded,.clean{background:#064e3b;color:#a7f3d0}.bad,.failed,.canceled,.infected{background:#7f1d1d;color:#fecaca}.running{background:#1e3a8a;color:#bfdbfe}.pending,.unknown{background:#78350f;color:#fde68a}main{padding:0 1rem 2rem}.panel,.result-card{background:#151c2e;border:1px solid #283044;border-radius:12px;padding:1.1rem;margin:1rem 0;box-shadow:0 10px 30px #0004}.section-head,.upload-row,.member-head,.result-hero{display:flex;gap:1rem;align-items:center;justify-content:space-between}.upload-row{justify-content:flex-start;margin-top:1rem;flex-wrap:wrap}button,input::file-selector-button{border:0;border-radius:8px;padding:.65rem .9rem;background:#2563eb;color:white;font-weight:600;cursor:pointer}.secondary,input::file-selector-button{background:#374151}button:disabled{opacity:.55;cursor:default}.status{min-height:1.4rem;margin-top:.8rem}.table-wrap{overflow:auto;margin-top:1rem}table{width:100%;border-collapse:collapse}th,td{text-align:left;border-bottom:1px solid #283044;padding:.7rem .45rem;white-space:nowrap}code,pre{font-family:ui-monospace,SFMono-Regular,Menlo,monospace}.result-hero{background:#0f172a;border-radius:10px;padding:1rem;margin-top:1rem;align-items:flex-start}.eyebrow{display:block;color:#9ca3af;font-size:.8rem;text-transform:uppercase;letter-spacing:.08em}.verdict{margin-top:.4rem}.hero-meta{min-width:55%}.kv{display:grid;grid-template-columns:8rem 1fr;gap:.7rem;padding:.25rem 0}.kv span{color:#9ca3af}.kv code{overflow-wrap:anywhere}.result-grid{display:grid;grid-template-columns:1fr 1fr;gap:1rem}.result-card{margin:1rem 0;box-shadow:none}.wide{width:100%}details{border-top:1px solid #283044;padding:.55rem 0}summary{cursor:pointer;font-weight:600}pre{white-space:pre-wrap;overflow-wrap:anywhere;background:#0b1020;padding:1rem;border-radius:8px;max-height:50vh;overflow:auto}.raw{margin-top:1rem}.alert{border-left:4px solid #dc2626;background:#450a0a;padding:.8rem 1rem;margin:1rem 0}.members{display:grid;gap:.7rem;margin-top:.7rem}.member{border:1px solid #283044;border-radius:8px;padding:.75rem}.member .members{margin-left:1rem}.detection,.bad-text{color:#fecaca}.hidden{display:none}footer{padding:0 1rem 2rem;color:#6b7280;font-size:.85rem}@media(max-width:700px){header{padding-top:1.2rem}.section-head,.result-hero{align-items:flex-start}.result-grid{grid-template-columns:1fr}.hero-meta{min-width:0}.kv{grid-template-columns:6rem 1fr}th:nth-child(3),td:nth-child(3){display:none}}`

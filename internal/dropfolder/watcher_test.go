...
func TestWatcherPollSubmitsStableSnapshotOnce(t *testing.T) {
	...
	if err := w.poll(t0, observed); err != nil {
		t.Fatal(err)
	}
	if err := w.poll(t0.Add(time.Second), observed); err != nil {
		t.Fatal(err)
	}
	if len(submitted) != 0 {
		t.Fatalf("submitted before stability window: %v", submitted)
	}
	if err := w.poll(t0.Add(2*time.Second), observed); err != nil {
		t.Fatal(err)
	}
	if len(submitted) != 1 {
	...
	if err := w.poll(t0.Add(10*time.Second), observed); err != nil {
		t.Fatal(err)
	}
	if len(submitted) != 1 {
...
func TestWatcherPollResetsWhenFileChanges(t *testing.T) {
	...
	if err := w.poll(t0.Add(4*time.Second), observed); err != nil {
		t.Fatal(err)
	}
	if submits != 1 {

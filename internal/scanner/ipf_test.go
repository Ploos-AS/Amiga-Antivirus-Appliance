...
func TestDetectFormatIPFCandidate(t *testing.T) {
	if got := DetectFormat("disk.ipf", []byte("synthetic-ipf"), 13); got != "ipf" {
		t.Fatalf("got %q", got)
	}
}

func TestScanIPFFeedsDerivedADFThroughPipeline(t *testing.T) {
	...
	helper := filepath.Join(dir, "ipf-helper")
	header := fmt.Sprintf(`{"schema":1,"format":"ipf","decoder":"fake-caps","decoder_version":"1.0","platform":"amiga","tracks":160,"derived_size":%d,"lossless_for_sector_scan":true}`, len(derived))
	...
	t.Setenv("AAA_IPF_HELPER", helper)

	inputPath := filepath.Join(dir, "disk.ipf")
	if err := os.WriteFile(inputPath, []byte("synthetic-ipf-candidate"), 0600); err != nil {
		t.Fatal(err)
	}
	...
	if got.Format != "ipf" || got.PreservationImage == nil {
		t.Fatalf("missing IPF preservation analysis: %+v", got)
	}
	if got.PreservationImage.Decoder != "fake-caps" || got.PreservationImage.DerivedSectorImage == nil {
...
func TestScanIPFRequiresHelper(t *testing.T) {
	dir := t.TempDir()
	inputPath := filepath.Join(dir, "disk.ipf")
	if err := os.WriteFile(inputPath, []byte("synthetic-ipf-candidate"), 0600); err != nil {
		t.Fatal(err)
	...
	if _, err := ScanFile(inputPath); err == nil {
		t.Fatal("expected missing IPF helper to fail closed")
	}
}

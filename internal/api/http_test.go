...
func (f fakeHistory) LoadLatest() ([]daemon.Job, error) { return f.jobs, f.err }
...
func TestHealthAndVersion(t *testing.T) {
	...
	for _, tc := range []struct{ path, want string }{{"/healthz", `"status":"ok"`}, {"/version", `"api_version":"v1"`}} {
...
func TestListAndGetScan(t *testing.T) {
	...
	jobs := []daemon.Job{{ID: "scan-1", Path: "/private/incoming/sample.adf", State: daemon.StateSucceeded, SubmittedAt: now, Result: &result}}
...
func TestResultsAndMissingResult(t *testing.T) {
	...
	h := NewHandler(fakeHistory{jobs: []daemon.Job{{ID: "done", State: daemon.StateSucceeded, SubmittedAt: time.Now().UTC(), Result: &result}, {ID: "pending", State: daemon.StatePending, SubmittedAt: time.Now().UTC()}}}, "test")
...
func TestSubmitStreamsIntoControlledIncomingRoot(t *testing.T) {
	...
	h := NewHandlerWithSubmission(fakeHistory{}, "test", SubmissionConfig{Submitter: submitter, IncomingRoot: root, MaxUploadBytes: 1024})
...
func TestSubmitRejectsTraversalAndOversize(t *testing.T) {
	root := t.TempDir()
	submitter := &fakeSubmitter{}
	h := NewHandlerWithSubmission(fakeHistory{}, "test", SubmissionConfig{Submitter: submitter, IncomingRoot: root, MaxUploadBytes: 4})
...
func TestSubmitQueueFailureIsServiceUnavailable(t *testing.T) {
	h := NewHandlerWithSubmission(fakeHistory{}, "test", SubmissionConfig{Submitter: &fakeSubmitter{err: errors.New("queue full")}, IncomingRoot: t.TempDir(), MaxUploadBytes: 1024})

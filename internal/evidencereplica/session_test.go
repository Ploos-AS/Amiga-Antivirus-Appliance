...
func TestSessionIDIndependentOfOrderAndTime(t *testing.T) {
	...
	if s1.SessionID != s2.SessionID {
		t.Fatalf("session IDs differ: %s != %s", s1.SessionID, s2.SessionID)
	}
	if s1.Objects[0].ObjectKind != KindEvidenceBundle {
		t.Fatalf("objects not sorted: %#v", s1.Objects)
	}
}
...
func TestSessionIDIgnoresResults(t *testing.T) {
	...
	if id1 != id2 {
		t.Fatalf("result changed intent identity")
	}
}

func TestSessionRejectsDuplicateIntent(t *testing.T) {
	a := sessionTestObject(t, KindEvidenceBundle, strings.Repeat("a", 64), ResultReplicated)
	if _, err := BuildSession([]SessionObject{a, a}, time.Unix(0, 0), ""); err == nil {
...
func TestSessionDeterministicRoundTrip(t *testing.T) {
	...
	if decoded.SessionID != s.SessionID {
		t.Fatal("round trip changed session ID")
	}
}
...
func TestSessionStrictDecodeRejectsUnknownField(t *testing.T) {
	...
	if _, err := DecodeSessionStrict([]byte(bad)); err == nil {
...
func TestSessionRejectsNonCanonicalTime(t *testing.T) {
	...
	if err := s.Validate(); err == nil {
		t.Fatal("expected non-UTC timestamp rejection")
	}
}
...
func TestSessionFailedObjectRequiresPortableError(t *testing.T) {
	...
	if err := s.Validate(); err == nil {
		t.Fatal("expected host-path error rejection")
	}
}
...
func TestSessionRejectsUnsortedObjects(t *testing.T) {
	...
	if err := s.Validate(); err == nil {
		t.Fatal("expected deterministic-order rejection")
	}
}

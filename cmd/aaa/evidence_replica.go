...
func runEvidenceReplicate(args []string, stdout, stderr io.Writer, now func() time.Time) error {
	...
	if fs.NArg() != 1 || *destination == "" || *kind == "" {
		return errors.New("requires --destination <dir> --kind <kind> <source>")
	}
	source := fs.Arg(0)
	...
	if err := replicateObject(source, *destination, receipt); err != nil {
		return err
	}
	...
	out := *receiptPath
	if out == "" {
		out = source + ".replica.json"
	}
	if err := writeNewFile(out, data, 0o640); err != nil {
		return err
	}
...
func runEvidenceReplica(args []string, stdout, stderr io.Writer) error {
	...
	if fs.NArg() != 1 || *root == "" {
		return errors.New("requires --root <dir> <receipt.json>")
	}
	...
	if sha != receipt.SHA256 || size != receipt.Size {
		return errors.New("replica object does not match receipt")
	}
...
func replicateObject(source, root string, receipt evidencereplica.Receipt) error {
	if err := ensureReplicaDirectories(root, receipt.SHA256); err != nil {
		return err
	}
	...
	if !opened.Mode().IsRegular() || !current.Mode().IsRegular() || !os.SameFile(opened, current) {
		return errors.New("source must remain the same regular file while opened")
	}
	...
	if n != opened.Size() || finished.Size() != opened.Size() || !finished.ModTime().Equal(opened.ModTime()) {
		return errors.New("source changed while replicating")
	}
	if n != receipt.Size || hex.EncodeToString(h.Sum(nil)) != receipt.SHA256 {
		return errors.New("replicated bytes do not match receipt")
	}
	...
	if sha != receipt.SHA256 || size != receipt.Size {
		return errors.New("published replica object failed verification")
	}
...
func verifyReplicaRoot(root string) error {
	...
	if !info.IsDir() || info.Mode()&os.ModeSymlink != 0 {
		return errors.New("replica root must be a real directory")
	}
	return nil
}
